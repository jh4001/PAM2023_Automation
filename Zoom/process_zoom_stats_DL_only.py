...
def get_image_frames(video):
    ...
    os.system('ffmpeg -i {video} -vf "select=not(mod(n\,42))" -vsync vfr -f image2 /tmp/images/zoomstats-%d.png 2> /dev/null'.format(video=video))
...
def process_frame(filename, current_time):
    ...
    stats = pytesseract.image_to_string(filename, config='--psm 12')
    # bw_stats = pytesseract.image_to_string(filename)
    bw_stats = stats

    # first we check if bandwidth frame
    bandwidth_matches = bandwidth_matcher.findall(bw_stats)
    rx_bandwidth_matches = rx_bandwidth_matcher.findall(bw_stats)
    if len(rx_bandwidth_matches) > 0: 
        video_stats[DL][BW_T].append(current_time)
        # handle the upload
        if len(rx_bandwidth_matches) == 1:
            try:
                value = float(rx_bandwidth_matches[0].split()[0])
                unit = rx_bandwidth_matches[0].split()[1]
                if "mb" in unit.lower():
                    value = value * 1000
                video_stats[DL][BW].append(value)
                most_recent_stat[DL][BW] = value
            except:
                video_stats[DL][BW].append(most_recent_stat[DL][BW])
                problem_frames[DL][BW] += 1
        else:
            video_stats[DL][BW].append(most_recent_stat[DL][BW])
            problem_frames[DL][BW] += 1
    ...
    # if not a bandwidth frame
        ...
        video_stats[DL][T].append(current_time)
        ...
        # process latency separately
        # print(latencies)
        if len(latencies) == 4:
            try:
                video_stats[DL][RTT].append(int(latencies[2]))
                most_recent_stat[DL][RTT] = int(latencies[2])
                # print(int(latencies[2]))
            except Exception as e:
                print(e)
                video_stats[DL][RTT].append(most_recent_stat[DL][RTT])
                problem_frames[DL][RTT] += 1
        else:
            # print(len(latencies))
            video_stats[DL][RTT].append(most_recent_stat[DL][RTT])
            problem_frames[DL][RTT] += 1
